...
model = Sequential()
model.add(Convolution2D(4, 4, 4, input_shape=(1, 6, 6), activation='relu', border_mode='same'))
...
model_checkpoint = ModelCheckpoint(filepath='saved_models/same_shape_model_2/weights-loss_{val_loss:.2f}-acc_{val_acc:.2f}.hdf5')

(val_input_data, val_target_data, _) = validation_data.get_validation_data_tuple()

model.load_weights("saved_models/same_shape_model_2/weights-loss_0.02-acc_0.58.hdf5")
